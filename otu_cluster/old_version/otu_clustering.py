...
#   [3] index seq 2
def merge_pe(F1_seq_list:list[str], F2rc_seq_list:list[str]):
    ...
    num_of_UF1 = len(F1_seq_list)
    num_of_UF2 = len(F2rc_seq_list)
    merged_sequences_info = []
    for i in range(num_of_UF1): 
        for j in range(num_of_UF2):
            merg_seq = merging(F1_seq_list[i], F2rc_seq_list[j])
            if merg_seq:
...
if __name__ == "__main__":
    ...
    # FILTERING delle sequenze con ELEVATO ERRORE e delle sequenze DUPLICATE con quality score più basso
    ...
    n_F1seq = len(F1_valid_seq)
    print ("\tNumber of valid F1 seq:", n_F1seq)
    ...
    n_F2seq = len(F2_valid_seq)
    print ("\tNumber of valid F2 seq:", n_F2seq)
